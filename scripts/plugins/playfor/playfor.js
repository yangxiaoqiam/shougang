...
var setAmount = {
    ...
    reduce:function(obj) {
        ...
        var data_minval = $(obj).attr("data-minval");
        if(data_minval){
            this.min = parseInt(data_minval, 10);
        }
        if (x >= this.min) {
            $(obj).val(x);
            recalc();
        } else {
            whir_alert("数值最小为" + this.min);
            $(obj).val(this.min);
            $(obj).focus();
        }
    },
    add:function(obj) {
        ...
        var data_maxval = $(obj).attr("data-maxval");
        if(data_maxval){
            this.max = parseInt(data_maxval, 10);
        }
        if (x <= this.max) {
            $(obj).val(x);
            recalc();
        } else {
            whir_alert("数值最大为" + this.max);
            $(obj).val(this.max);
            $(obj).focus();
        }
    },
