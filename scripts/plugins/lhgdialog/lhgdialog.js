...
var _ie6 = isIE && check(/msie 6/),/*window.ActiveXObject && !window.XMLHttpRequest,*/
	_fn = function(){},
	_count = 0,
	_rurl = /^url:/,
	_singleton,
							...
							'<td colspan="2">' +
								'<div class="ui_title_bar">' +
									...
									'<div class="ui_title_buttons">' +
										'<a class="ui_min" href="javascript:void(0);" title="'+comm.lhg_min+'"><b class="ui_min_b"></b></a>' +
										'<a class="ui_max" href="javascript:void(0);" title="'+comm.lhg_max+'"><b class="ui_max_b"></b></a>' +
										'<a class="ui_res" href="javascript:void(0);" title="'+comm.lhg_res+'"><b class="ui_res_b"></b><b class="ui_res_t"></b></a>' +
										'<a class="ui_close" href="javascript:void(0);" title="'+comm.lhg_close+'(esc'+comm.lhg_key+')">\xd7</a>' +
									'</div>' +
								'</div>' +
							'</td>' +
						'</tr>' +
...
if( !$.support.leadingWhitespace /*$.browser.msie*/ ) {
    if(_top.frames && _top.frames.length > 0) {
        _hasFrames = true;
        _$top = $(_top.document.body);
    }
}
...
var lhgdialog = function( config )
{
	...
	//lhgdialog.setting.zIndex =  lhgdialog.setting.zIndex +1;
	...
	//config.ldg_lockmask_divId=config.ldg_lockmask_divId||"ldg_lockmask"+config.id;
	...
	config.button.push({
		...
		name: config.okVal,
		callback: config.ok,
		focus: !lhgdialog.ispad()&&config.focus
	});
	...
	//alert(lhgdialog.setting.zIndex);

	config.ldg_lockmask_divId=config.ldg_lockmask_divId||"ldg_lockmask"+lhgdialog.setting.zIndex;
...
/**
判断是否是ipad打开
不调用公用的方法 是防止有的页面没加载公用的js 文件 **/
lhgdialog.ispad=function(){
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	content: function( msg, add, frm )
	{
		...
		//zhangmm
		var whirIframe =  '' ;
		//alert($("object[classid='clsid:8B23EA28-2009-402F-92C4-59BE0E063499'],object[classid='clsid:2294689C-9EDF-40BC-86AE-0438112CA439'],object[classid='clsid:D85C89BE-263C-472D-9B6B-5264CD85B36E']").length>0);
		if($("object[classid='clsid:8B23EA28-2009-402F-92C4-59BE0E063499'],object[classid='clsid:2294689C-9EDF-40BC-86AE-0438112CA439'],object[classid='clsid:D85C89BE-263C-472D-9B6B-5264CD85B36E']").length>0){
			whirIframe =  '<iframe  width="97.3%" height="95.8%"  frameborder="0" style="top:9px;left:9px;z-index:-1;position:absolute;filter:alpha(opacity=0);opacity:0;-ms-filter:\'progid:DXImageTransform.Microsoft.Alpha(Opacity=0)\';"></iframe>' ;
		}
	...
	size: function( width, height )
	{
        ...
        var maxWidth, scaleWidth,
            main = DOM.main;
        if (width) {
			that._lockDocW = width.toString().indexOf('%') !== -1 ? width : null;
			maxWidth = _$top.width() - wrap.offsetWidth + main[0].offsetWidth;
			scaleWidth = that._toNumber(width, maxWidth);
			width = scaleWidth;
			
			if (typeof width === 'number') {
				style.width = 'auto';
				main[0].style.width = width + 'px';
				style.width = wrap.offsetWidth + 'px'; // 防止未定义宽度的表格遇到浏览器右边边界伸缩
			} else if (typeof width === 'string') {
				style.width = width;
				width === 'auto' && DOM.wrap.css('width', 'auto');
			};
		};
	...
	show: function( args )
	{
	    ...
	    var _ldg_lockmask_divId=this.config.ldg_lockmask_divId||"ldg_lockmask";
		if( !args && this._lock )
		    $('#'+_ldg_lockmask_divId,_doc)[0].style.display = '';
	...
	{   var _ldg_lockmask_divId=this.config.ldg_lockmask_divId||"ldg_lockmask";
		...
		if( !args && this._lock )
		    $('#'+_ldg_lockmask_divId,_doc)[0].style.display = 'none';
	...
	{   var _ldg_lockmask_divId=this.config.ldg_lockmask_divId||"ldg_lockmask";
		var that = this, frm,
			...
			config = that.config,
			mask = $('#'+_ldg_lockmask_divId,_doc)[0],
		...
		if( !mask )
		{
			...
			mask.id = _ldg_lockmask_divId;
			mask.style.cssText = 'position:' + positionType + ';left:0;top:0;width:100%;height:100%;overflow:hidden; background:#DCE2F1;filter:alpha(opacity=60);opacity:.6;';
	...
	{   
		...
		{
		    // 无限级锁屏
		    //alert("2:"+(config.parent && config.parent._lock));
	...
	close: function()
	{
		...
		if( that.iframe )
		{
			...
			$(that.iframe).unbind('load',that._fmLoad).attr('src',"about:blank").remove();//.attr('src',"javascript:''").remove();
	...
	_iframe: function( url )
	{
		...
		$(whirIframe).insertBefore($iframe);
	...
	_reset: function()
	{   
		var _ldg_lockmask_divId="ldg_lockmask";
		if(this.config){
			if(this.config.ldg_lockmask_divId){
		         _ldg_lockmask_divId=this.config.ldg_lockmask_divId
			}
		}
		...
		if(test)
		{
			//IE6下遮罩大小改变
			if( that._lock && _ie6 )
			    $('#'+_ldg_lockmask_divId,_doc).css({ width:tw + 'px', height:tt + 17 + 'px' });
...
_top != window && $(window).bind('unload',function()
{
	...
	{
	    if(list[i]){   
			var _ldg_lockmask_divId=list[i].ldg_lockmask_divId||"ldg_lockmask";
		    list[i].close();	
			$('#'+_ldg_lockmask_divId,_doc)[0] && $('#'+_ldg_lockmask_divId,_doc).remove();
		}
	}
...
lhgdialog.setting =
{
    content: '<center><div class="ui_loading"><span>loading...</span></div></center>',
	title: comm.lhg_tip,     			// 标题,默认'视窗'
	button: null,	     		// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数
	init: null,					// 对话框初始化后执行的函数
	close: null,				// 对话框关闭前执行的函数
	okVal: comm.confirm,				// 确定按钮文本,默认'确定'
	cancelVal: comm.cancel,			// 取消按钮文本,默认'取消'
	skin: '',					// 多皮肤共存预留接口
	esc: true,					// 是否支持Esc键关闭
	...
	max: true,                  // 是否显示最大化按钮
	min: true,                  // 是否显示最小化按钮
	zIndex: 70000,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
$(function(){
	...
	{
	    if(_count) return;
		lhgdialog({left:'-9999em',time:10000,fixed:false,lock:false,focus:false});
	},150);
...
lhgdialog.alert = function( content, callback, parent )
{
	...
	return $.dialog({
		...
		lock: true,
		cache: false, 
		content: '<div style="width:380px;padding:5px;font-size:15px;">'+content+'</div>',
		ok: true,
		resize: false,
		close: callback,
		parent: parent || null
 ...
 * @param	{String}	消息内容
 */
lhgdialog.alertWithTitle = function( title,content, callback, parent )
{ 
	return $.dialog({
		title: title,
		id: 'alertWithTitle',
		...
		lock: true,
		cache: false, 
		content: '<div style="width:600px;padding:5px;font-size:15px;">'+content+'</div>',
		ok: true,
		resize: false,
		close: callback,
		parent: parent || null
...
/**
 * 警告
 * 流程设计器 里原始whir_alert显示不正常， 特加次函数
 * @param	{String}	消息内容
 */
lhgdialog.alert_wf = function( content, callback, parent ,left,top)
{
 ...
 * @param	{String}	消息内容
 */
lhgdialog.alert_eh = function( content, callback, parent,width,height )
{
	...
	return $.dialog({
		...
		lock: true,
		cache: false, 
		content: '<div style="width:380px;height:100px;line-height:100px;font-size:15px;">'+content+'</div>',
		ok: true,
		resize: false,
		close: callback,
		parent: parent || null
...
lhgdialog.confirm = function( content, yes, no, parent )
{
	//为解决iframe嵌套时提示层位置问题
	var win = window;try{
	if(frameElement!=null && frameElement.api==undefined && self.frameElement!=null && self.frameElement.tagName=="IFRAME" && self.frameElement.name!='mainFrame'  ){
		win = window.parent;
	}}catch(e){}
	//if(frameElement.api){
	...
	return win.lhgdialog({
		...
		lock: true,
		width:230,
		content: '<div style="width:380px;padding:25px;font-size:15px;">'+content+'</div>',
		resize: false,
		parent: parent || null,
		ok: function(here){
 ...
 * @param	{Function}	取消按钮回调函数
 */
lhgdialog.confirm_wf = function( content, yes, no, parent ,left,top)
{
 ...
 * @param	{String}	默认值
 */
lhgdialog.whir_prompt = function( content, yes, value, title,parent )
{
	value = value || '';
	var input;
	return lhgdialog({
 ...
 */
lhgdialog.tips = function( content, time, icon, callback, lock_)
{
	...
	.content('<div style="padding-top:2px;">'+content+'</div>')
